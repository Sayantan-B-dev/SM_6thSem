...
```mermaid
graph TD
A[Change in Market Condition] --> B{What Changed?}
B -->|Own Price Changed| C[Move Along Same Curve]
B -->|Other Factors Changed| D[Shift Entire Curve]
C --> E[Change in Quantity Demanded]
D --> F[Change in Demand]
E --> G[Extension or Contraction]
F --> H[Increase or Decrease in Demand]
```
