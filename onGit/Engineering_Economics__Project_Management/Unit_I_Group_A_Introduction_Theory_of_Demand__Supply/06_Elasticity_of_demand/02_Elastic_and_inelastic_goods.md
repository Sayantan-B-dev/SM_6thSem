...
```mermaid
graph TD
A[Price Change] --> B[Calculate % change in Qd]
B --> C[Compute E_d = %ΔQd / %ΔP]
C --> D{E_d value}
D -->|E_d = 0| E[Perfectly Inelastic]
D -->|0 < E_d < 1| F[Inelastic Good]
D -->|E_d = 1| G[Unitary Elastic]
D -->|E_d > 1| H[Elastic Good]
D -->|E_d = ∞| I[Perfectly Elastic]
E --> J[Revenue rises with price hike]
F --> K[Revenue rises with price hike]
H --> L[Revenue falls with price hike]
```
