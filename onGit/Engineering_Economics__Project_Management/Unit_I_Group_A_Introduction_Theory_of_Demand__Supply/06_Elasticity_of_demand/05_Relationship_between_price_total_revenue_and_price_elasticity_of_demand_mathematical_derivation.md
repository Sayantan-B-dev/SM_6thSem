...
```mermaid
graph TD
A[Price Change] --> B[Quantity Demanded Changes]
B --> C{Elasticity of Demand}
C -->|Elastic - E_d - > 1| D[MR Positive]
D --> E[TR Moves Opposite to Price]
C -->|Unitary - E_d - = 1| F[MR Zero]
F --> G[TR Unchanged / Maximum]
C -->|Inelastic - E_d - < 1| H[MR Negative]
H --> I[TR Moves Same Direction as Price]
```
