...
```mermaid
graph TD
A[Output Q] --> B[TFC - Horizontal Line]
A --> C[TVC - Starts at 0, Rises with Q]
C --> D[TC = TFC + TVC]
B --> D
D --> E[AC = TC / Q]
C --> F[AVC = TVC / Q]
B --> G[AFC = TFC / Q]
D --> H[MC = Change in TC / Change in Q]
H --> I[MC curve - U shaped]
E --> J[AC curve - U shaped]
F --> K[AVC curve - U shaped]
H --> L[MC cuts AC & AVC at their minimum]
G --> M[AFC falls continuously]
```
