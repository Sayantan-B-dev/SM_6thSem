...
```mermaid
graph TD
A[Fixed Inputs] --> B[Total Fixed Cost TFC]
C[Variable Inputs] --> D[Total Variable Cost TVC]
B --> E[Total Cost TC = TFC + TVC]
D --> E
E --> F[Average Total Cost ATC = TC/Q]
B --> G[Average Fixed Cost AFC = TFC/Q]
G --> H[AFC falls as Q rises]
D --> I[Average Variable Cost AVC = TVC/Q]
F --> J[ATC = AFC + AVC]
I --> K[Marginal Cost MC]
K --> L[MC cuts AVC and ATC at minimum]
```
...
```mermaid
graph TD
M[Choice of Plant Size] --> N[Multiple SRAC Curves]
N --> O[Long-run Average Cost LRAC = Envelope of SRACs]
O --> P[LRAC is U-shaped due to economies/diseconomies of scale]
P --> Q[Long-run Marginal Cost LMC]
Q --> R[LMC cuts LRAC at its minimum point]
```
