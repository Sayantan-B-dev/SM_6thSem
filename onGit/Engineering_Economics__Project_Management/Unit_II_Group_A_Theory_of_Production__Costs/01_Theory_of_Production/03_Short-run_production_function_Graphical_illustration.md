...
```mermaid
graph TD
A[Fixed Input + Variable Input Labour] --> B[Total Product TP Curve]
A --> C[Average Product AP Curve]
A --> D[Marginal Product MP Curve]
B --> B1[TP: S-shaped, first convex then concave, peaks at max]
C --> C1[AP: Inverted U-shape, max where MP = AP]
D --> D1[MP: Inverted U-shape, cuts AP at its maximum, becomes zero when TP peaks, negative in Stage III]
B1 --> E[Stage I: Increasing Returns]
B1 --> F[Stage II: Diminishing Returns]
B1 --> G[Stage III: Negative Returns]
```
