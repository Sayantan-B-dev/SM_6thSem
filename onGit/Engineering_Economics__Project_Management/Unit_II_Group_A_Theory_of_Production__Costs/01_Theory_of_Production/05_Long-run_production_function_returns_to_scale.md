...
```mermaid
graph TD
A[Increase All Inputs by Same Percentage] --> B{Compare Output Increase}
B --> C[Output Rises More Than Inputs]
B --> D[Output Rises Same as Inputs]
B --> E[Output Rises Less Than Inputs]
C --> F[Increasing Returns to Scale]
D --> G[Constant Returns to Scale]
E --> H[Decreasing Returns to Scale]
```
