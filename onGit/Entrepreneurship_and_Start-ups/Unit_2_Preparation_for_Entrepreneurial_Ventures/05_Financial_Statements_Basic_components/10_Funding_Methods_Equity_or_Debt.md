...
```mermaid
graph TD
    A[Funding Need] --> B{Business Stage & Risk}
    B -->|Early Stage / High Risk| C[Equity Funding]
    B -->|Stable Cash Flow / Assets| D[Debt Funding]
    C --> C1[Angel Investors]
    C --> C2[Venture Capital]
    D --> D1[Term Loan]
    D --> D2[Working Capital Loan]
    E{Need for Control?} -->|Want Full Control| D
    E -->|Okay to Share| C
    D --> F[Regular EMI & Interest]
    C --> G[Dividends & Exit]
    F --> H[Balanced Capital Structure]
    G --> H
```
