...
```mermaid
graph TD
    A[Net Profit from Income Statement] --> B[Adjust for Non-Cash Items & Working Capital Changes]
    B --> C[Cash from Operating Activities]
    D[Purchase/Sale of Fixed Assets] --> E[Cash from Investing Activities]
    F[Loans, Equity, Dividends] --> G[Cash from Financing Activities]
    C --> H[Net Increase/Decrease in Cash]
    E --> H
    G --> H
    H --> I[Closing Cash Balance]
```
