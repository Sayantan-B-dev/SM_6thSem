...
```mermaid
graph TD
A[Business Spending] --> B{Benefit > 1 Year?}
B -->|Yes| C[Capital Expenditure]
B -->|No| D[Revenue Expenditure]
C --> E[Show as Asset in Balance Sheet]
E --> F[Charge Depreciation to P&L each year]
D --> G[Charge full amount to P&L in current year]
```
