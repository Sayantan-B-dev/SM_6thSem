...
```mermaid
graph TD
A[Debtor unable to pay debts] --> B[File bankruptcy application]
B --> C[Adjudicating authority admits application]
C --> D[Moratorium declared]
D --> E[Insolvency professional appointed]
E --> F[Creditors submit claims]
F --> G[Committee of creditors formed]
G --> H{Resolution plan approved?}
H -->|Yes| I[Plan implemented, business revived]
H -->|No| J[Liquidation of assets]
J --> K[Proceeds distributed to creditors]
I --> L[Debtor discharged]
K --> L
L --> M[Fresh start for entrepreneur]
```
