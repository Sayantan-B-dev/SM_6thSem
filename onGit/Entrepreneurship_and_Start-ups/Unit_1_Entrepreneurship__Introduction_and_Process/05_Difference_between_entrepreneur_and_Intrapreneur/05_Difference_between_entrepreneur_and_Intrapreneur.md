...
```mermaid
graph TD
A[Idea Generation] --> B{Who takes initiative?}
B -->|Independent| C[Entrepreneur]
B -->|Inside Company| D[Intrapreneur]
C --> E[Self-funded, Full Risk]
C --> F[Owns Business, Keeps Profits]
D --> G[Company-funded, Shared Risk]
D --> H[Company Owns Venture, Gets Salary + Bonus]
```
