...
```mermaid
graph TD
    A[Startup with Idea] --> B[Apply to Govt Incubation Centre]
    B --> C[Application Screening & Pitch]
    C --> D[Selection & Agreement]
    D --> E[Physical Space & Infrastructure]
    D --> F[Mentorship & Training]
    D --> G[Financial Linkages & Grants]
    E --> H[Product Development & Testing]
    F --> H
    G --> H
    H --> I[Graduation after Incubation Period]
    I --> J[Independent Business Operation]
```
