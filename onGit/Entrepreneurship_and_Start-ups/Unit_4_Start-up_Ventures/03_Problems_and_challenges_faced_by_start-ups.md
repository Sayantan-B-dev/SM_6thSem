...
```mermaid
graph TD
    A[Start-up Launch] --> B[Insufficient Capital]
    A --> C[Unknown Brand]
    A --> D[Small Team]
    B --> E[Cash Flow Crunch]
    C --> F[Low Customer Acquisition]
    D --> G[Skill Gaps & Burnout]
    E --> H{Runway Ends?}
    F --> I[Revenue Below Target]
    G --> J[Product Delays]
    H -->|Yes| K[Start-up Failure]
    I --> L[Investor Confidence Drops]
    J --> L
    L --> H
```
